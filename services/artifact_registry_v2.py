...
ARTIFACT_TYPES = {
    ...
    "repaired_transcript": ArtifactTypeMeta("repaired_transcript", "Repaired Transcript", "human", "⌁", 25, "markdown", "Transcript repaired with glossary and quality layer."),
...
def sort_artifacts(artifacts: list[dict]) -> list[dict]:
    return sorted(artifacts, key=lambda item: get_artifact_meta(item.get("artifact_type", "")).order)
