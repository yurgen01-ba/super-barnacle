...
def render_user_artifact_generator(
    *,
    ...
    instruction = st.text_area(
        "Что должен подготовить AI",
        value=default_instruction,
        height=150,
        key=f"{key_prefix}_instruction",
    )

    if not st.button("Сгенерировать", key=f"{key_prefix}_generate", type="primary", width="stretch"):
        return

    if not title.strip() or not instruction.strip():
        st.warning("Укажите название и задачу для AI.")
        return

    settings = workspace_repository.get_settings(project_id)
    provider_name = settings.get("transcript_extractor_provider", "ollama")
    if provider_name != "ollama":
        st.error("Для генерации артефактов сейчас поддерживается локальный провайдер Ollama. Выберите его в настройках проекта.")
        return
    ...
    try:
        ...
        st.success("Артефакт создан и добавлен в «Экспорт данных».")
        st.markdown(content)
        st.download_button(
            "Скачать Markdown",
            data=content,
...
def render_generic_artifact_generator(project_id: str):
    artifact_type = st.selectbox(
        ...
        format_func=lambda value: ARTIFACT_PRESETS[value][0],
        key="ui_v2_generic_artifact_type",
    )
    title, instruction = ARTIFACT_PRESETS[artifact_type]
