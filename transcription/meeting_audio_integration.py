...
def process_audio_with_selected_backend(audio_path:str,language:str|None=None,min_speakers:int|None=2,max_speakers:int|None=6,progress_callback=None)->dict:
    if progress_callback: progress_callback({"event":"audio_intelligence_started","language":language})
    result=audio_intelligence_service.process(audio_path,language,min_speakers,max_speakers)
