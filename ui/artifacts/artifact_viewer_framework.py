...
def render_artifact_viewer(artifact: dict):
    ...
    c1.caption(f"{t('type')}: {artifact.get('artifact_type')}")
    c2.caption(f"{t('format')}: {artifact.get('format')}")
    with c3:
        render_artifact_download_button(artifact)
    content = artifact.get("content") or ""
    query = st.text_input(t("search_inside_artifact"), key=f"artifact_search_{artifact['id']}")
    if query:
    ...
    if (artifact.get("format") or meta.default_format) == "json":
        try:
            ...
            st.text_area(t("content"), content, height=560)
    else:
        st.text_area(t("content"), content, height=560, key=f"artifact_view_{artifact['id']}")
    with st.expander(t("artifact_metadata"), expanded=False):
